...
/// Firmware metadata for the OSD About screen.
...
typedef struct config {
    ...
    uint32_t audio_volume_percent; ///< Volume level used by the OSD audio controls.
    bool audio_mute;               ///< Whether the HDMI audio output is muted.
    uint32_t video_crop_x;         ///< Horizontal crop amount applied to the captured image.
    uint32_t video_crop_y;         ///< Vertical crop amount applied to the captured image.
    dvi_color_mode_t dvi_color_mode; ///< The DVI color mode (see @ref dvi_color_mode_t).
